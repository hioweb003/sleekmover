...
@section('content')
            ...
            <div class="card-header"><i class="fas fa-table mr-1"></i>Create Services </div>
                            <div class="card-body">
                                <div class="col-md-8">
                                    @include('includes.success')
                                    @include('includes.errors')  
                            <form action="{{route('ser.store')}}" method="post" enctype="multipart/form-data">
                                @csrf
                                 <div class="form-group">
                                 <input type="text" name="title" id="caption" class="form-control" placeholder="Enter Title" value="{{old('title')}}" autofocus>
                                     </div> 
                                     <div class="form-group">
                                        <input type="text" name="heading" id="caption" class="form-control" placeholder="Enter Heading" value="{{old('heading')}}">
                                            </div> 
